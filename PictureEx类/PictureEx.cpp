...
void CPictureEx::ThreadAnimation()
{
	...
	while (!m_bExitThread)
	{
		...
		//   3 - restore to previous
			...
			//m_arrFrames[m_nCurrFrame].m_frameSize.cx = GetSystemMetrics(SM_CXFULLSCREEN);
			//m_arrFrames[m_nCurrFrame].m_frameSize.cy = GetSystemMetrics(SM_CYFULLSCREEN);
			//////// disposal method #3
...
void CPictureEx::OnPaint() 
{
	...
	if (nPaintWidth > 0)
	{
		...
		::StretchBlt(dc.m_hDC, 0, 0, nPaintWidth,nPaintHeight, m_hMemDC, 0,0,m_PictureSize.cx, m_PictureSize.cy,SRCCOPY); 
	}
	else
	{

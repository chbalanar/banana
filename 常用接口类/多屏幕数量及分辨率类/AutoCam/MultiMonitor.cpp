...
//为监视器创建一个HDC，由客户机为正常的多监视器绘图调用DeleteDC，不必为每个监视器获取一个dc。
//	Windows负责正确绘制所有显示器
//	只有非常精确的应用程序才需要为每个监视器提供一个DC
HDC CMonitor::CreateDC() const
{
	ASSERT( IsMonitor() );		//匹配显示屏句柄

	CString name;
	GetName( name );			//获取显示屏信息

	//create a dc for this display
	HDC hdc = ::CreateDC( name, name, NULL, NULL );		//为显示屏创建一个HDC
	ASSERT( hdc != NULL );								//判断是否创建成功
...
void CMonitor::GetName( CString& string ) const		//获取显示屏名称
{
	ASSERT( IsMonitor() );

	MONITORINFOEX mi;
	mi.cbSize = sizeof( mi );
	::GetMonitorInfo( m_hMonitor, &mi );			//通过参数mi获取显示屏信息		//GetMonitorInfo函数返回一个显示器的信息参数为句柄,储存显示屏信息的结构体MONITORINFOEX的引用

	string = mi.szDevice;							//通过参数传递获取的字符串
}

...
BOOL CMonitors::IsMonitor( const HMONITOR hMonitor )	//判定给定句柄是否是有效显示屏
{
	if ( hMonitor == NULL )
		return FALSE;
...
BOOL CALLBACK CMonitors::FindMatchingMonitorHandle( HMONITOR hMonitor, HDC hdcMonitor, LPRECT lprcMonitor, LPARAM dwData )
{
	...
	if ( hMonitor == pMatch->target )
	{
		...
		pMatch->foundMatch = TRUE;				//匹配到了给定屏幕句柄
		return FALSE; //stop enumerating		//停止匹配
	}

	//haven't found a match yet
	pMatch->foundMatch = FALSE;					//没有匹配到给定句柄
	return TRUE;	//keep enumerating			//继续匹配
}
